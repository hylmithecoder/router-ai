...
export default function KeysPage() {
  ...
  async function create(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await api.createKey(name.trim(), Math.max(0, Number(quota) || 0));
        ...
        <Card title="Create key" className="lg:col-span-1">
          ...
          {newKey && (
            <div className="mt-4 rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3">
              ...
              <code className="break-all font-mono text-xs text-emerald-300">{newKey}</code>
              <button
          ...
          {keys && (
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-zinc-800">
                {keys.map((k) => (
                  <tr key={k.id}>
                    <td className="py-2.5 font-medium text-zinc-300">{k.name}</td>
                    <td className="py-2.5 font-mono text-xs text-zinc-500">{k.key_prefix}</td>
                    <td className="py-2.5 text-right font-mono text-zinc-400">
                      {k.quota_daily_tokens > 0 ? formatTokens(k.quota_daily_tokens) : "∞"}
                    </td>
