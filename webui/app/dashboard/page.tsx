...
import { BarChart, Card, ErrorBox, QuotaGauge, Spinner, Stat } from "@/components/ui";
            ...
            {summary && (
                <div className="flex flex-col gap-6">
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        />
                        <Stat label="Requests today" value={formatTokens(summary.today.requests)} />
                        <Stat
                            label="Providers used"
                            value={formatTokens(summary.by_provider.length)}
                            sub={summary.by_provider.map((p) => p.provider).join(", ") || "none"}
                        />
                        <Stat
                            label="Active keys"
                    ...
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                        ...
                        <Card title="Last 7 days (tokens)" className="lg:col-span-2">
                            {summary.by_day.length === 0 ? (
                                <p className="py-10 text-center text-sm text-zinc-500">no usage yet</p>
                            ) : (
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-zinc-800">
                                    {summary.by_key.map((k) => (
                                        <tr key={k.key_id}>
                                            <td className="py-2 font-medium text-zinc-300">{k.key_name}</td>
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-zinc-800">
                                    {summary.by_provider.map((p) => (
                                        <tr key={p.provider}>
                                            <td className="py-2 font-mono text-zinc-300">{p.provider}</td>
                                            <td className="py-2 text-right font-mono text-zinc-400">
