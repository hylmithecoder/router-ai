...
import { LuCopy, LuCheck, LuKey, LuTrash2, LuPower, LuPlus } from "react-icons/lu";
        ...
        <Card title="Create key" className="lg:col-span-1">
          <form onSubmit={create} className="flex flex-col gap-4">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 py-2.5 text-sm font-semibold text-white shadow-xs transition-all hover:bg-indigo-700 hover:shadow-sm disabled:opacity-40"
            >
              <LuPlus className="h-4 w-4" />
              <span>{busy ? "Creating key…" : "Generate API Key"}</span>
            </button>
          </form>
          ...
          {newKey && (
            <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50/80 p-4 shadow-xs">
              <div className="flex items-center gap-2 mb-1.5">
                <LuKey className="h-4 w-4 text-emerald-700 shrink-0" />
                <p className="text-xs font-bold text-emerald-800">
                  Key generated — copy now (will not be shown again):
                </p>
              </div>
                      ...
                      <td className="py-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          >
                            ...
                            <span>{k.enabled ? "Disable" : "Enable"}</span>
                          </button>
                          <button
                            onClick={() => remove(k)}
                            className="inline-flex items-center gap-1 text-xs font-semibold text-rose-600 hover:text-rose-800 hover:underline ml-2"
                            title="Delete Key"
                          >
